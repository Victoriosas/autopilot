...
export const ConnectorsDirectoryModal: React.FC<ConnectorsDirectoryModalProps> = ({ onClose }) => {
  ...
  const [cjMcpStatus, setCjMcpStatus] = useState<CjMcpStatus | null>(null);
  const [cjMcpLoading, setCjMcpLoading] = useState(false);

  const loadCjMcpStatus = async (probe = false) => {
    setCjMcpLoading(true);
    try {
      const res = await apiFetch(`/api/autopilot/v4/cj-mcp/status${probe ? '?probe=1' : ''}`);
      const data = await res.json().catch(() => ({ error: `HTTP_${res.status}` }));
      setCjMcpStatus(data);
    } catch (err: any) {
      setCjMcpStatus({ error: err?.message || 'CJ_MCP_STATUS_UNAVAILABLE' });
    } finally {
      setCjMcpLoading(false);
    }
  };

  useEffect(() => {
    void loadCjMcpStatus(false);
  }, []);
  ...
  const mcpReady = Boolean(cjMcpStatus?.configured && cjMcpStatus?.enabled && cjMcpStatus?.safetyReady);
  const probeOk = cjMcpStatus?.probe?.ok === true;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/80 backdrop-blur-xl overflow-y-auto animate-fadeIn">
      <div 
        className="relative w-full max-w-5xl bg-[#0d111d]/95 border border-white/10 rounded-2xl shadow-2xl overflow-hidden my-8 backdrop-blur-2xl flex flex-col max-h-[90vh]"
      ...
      >
        ...
        <div className="px-6 pt-5">
          <div className={`rounded-2xl border p-4 ${mcpReady ? 'bg-emerald-500/[0.06] border-emerald-500/25' : 'bg-amber-500/[0.05] border-amber-500/25'}`}>
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  <div className="font-bold text-sm text-white flex items-center gap-2">
                    <Cpu className="w-4 h-4 text-indigo-400" />
                    CJ Dropshipping · MCP Oficial
                  </div>
                  {mcpReady ? (
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-emerald-500/10 text-emerald-300 border border-emerald-500/30">
                      <CheckCircle2 className="w-3 h-3" /> READ-ONLY SHADOW READY
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-amber-500/10 text-amber-300 border border-amber-500/30">
                      <AlertTriangle className="w-3 h-3" /> CONFIGURACIÓN PENDIENTE
                    </span>
                  )}
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-[10px]">
                  <div className="rounded-lg bg-black/30 border border-white/5 px-2.5 py-2">
                    <div className="text-slate-500">Token MCP</div>
                    <div className={cjMcpStatus?.configured ? 'text-emerald-300 font-bold' : 'text-amber-300 font-bold'}>
                      {cjMcpStatus?.configured ? 'Configurado' : 'No detectado'}
                    </div>
                  </div>
                  <div className="rounded-lg bg-black/30 border border-white/5 px-2.5 py-2">
                    <div className="text-slate-500">Shadow MCP</div>
                    <div className={cjMcpStatus?.enabled ? 'text-emerald-300 font-bold' : 'text-amber-300 font-bold'}>
                      {cjMcpStatus?.enabled ? 'Habilitado' : 'Deshabilitado'}
                    </div>
                  </div>
                  <div className="rounded-lg bg-black/30 border border-white/5 px-2.5 py-2">
                    <div className="text-slate-500">Barreras</div>
                    <div className={cjMcpStatus?.safetyReady ? 'text-emerald-300 font-bold' : 'text-amber-300 font-bold'}>
                      {cjMcpStatus?.safetyReady ? 'Seguras' : 'Revisar'}
                    </div>
                  </div>
                  <div className="rounded-lg bg-black/30 border border-white/5 px-2.5 py-2">
                    <div className="text-slate-500">REST fallback</div>
                    <div className={cjMcpStatus?.restFallbackConfigured ? 'text-emerald-300 font-bold' : 'text-slate-400 font-bold'}>
                      {cjMcpStatus?.restFallbackConfigured ? 'Disponible' : 'No'}
                    </div>
                  </div>
                </div>
                {cjMcpStatus?.error && (
                  <div className="text-[10px] text-rose-300 font-mono">{cjMcpStatus.error}</div>
                )}
                {cjMcpStatus?.probe && (
                  <div className={`rounded-lg border px-3 py-2 text-[10px] ${probeOk ? 'bg-emerald-500/[0.05] border-emerald-500/20 text-emerald-200' : 'bg-rose-500/[0.05] border-rose-500/20 text-rose-200'}`}>
                    {probeOk ? (
                      <>
                        <div className="font-bold mb-1">Conexión oficial CJ MCP verificada</div>
                        <div>Herramientas descubiertas: {cjMcpStatus.probe.discovered?.length || 0} · Read-only disponibles: {cjMcpStatus.probe.availableReadOnly?.length || 0}</div>
                        {(cjMcpStatus.probe.availableReadOnly?.length || 0) > 0 && (
                          <div className="mt-1 font-mono text-emerald-300/90 break-words">
                            {cjMcpStatus.probe.availableReadOnly?.join(' · ')}
                          </div>
                        )}
                        {(cjMcpStatus.probe.missingExpected?.length || 0) > 0 && (
                          <div className="mt-1 text-amber-300">No reportadas por CJ: {cjMcpStatus.probe.missingExpected?.join(', ')}</div>
                        )}
                      </>
                    ) : (
                      <div className="font-mono">Prueba MCP falló: {cjMcpStatus.probe.error || 'CJ_MCP_PROBE_FAILED'}</div>
                    )}
                  </div>
                )}
              </div>
              ...
              <div className="flex gap-2 shrink-0">
                <button
                  type="button"
                  onClick={() => void loadCjMcpStatus(false)}
                  disabled={cjMcpLoading}
                  className="px-3 py-2 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 text-slate-200 text-[11px] font-bold disabled:opacity-50 flex items-center gap-2"
                >
                  <RefreshCw className={`w-3.5 h-3.5 ${cjMcpLoading ? 'animate-spin' : ''}`} />
                  Estado
                </button>
                <button
                  type="button"
                  onClick={() => void loadCjMcpStatus(true)}
                  disabled={cjMcpLoading || !mcpReady}
                  className="px-3 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-[11px] font-bold disabled:opacity-40 flex items-center gap-2"
                >
                  <ShieldCheck className="w-3.5 h-3.5" />
                  {cjMcpLoading ? 'Probando...' : 'Probar CJ MCP'}
                </button>
              </div>
            </div>
            <div className="mt-3 text-[10px] text-slate-500">
              Solo lectura en Shadow. Crear órdenes, agregar al carrito y demás escrituras MCP están bloqueadas por el backend antes de cualquier llamada a CJ.
            </div>
          </div>
        </div>

        {/* Modal Body: Split view */}
        <div className="flex-1 overflow-y-auto grid grid-cols-1 md:grid-cols-12 divide-y md:divide-y-0 md:divide-x divide-white/10 mt-5">
