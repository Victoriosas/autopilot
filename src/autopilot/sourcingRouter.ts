...
function normalizedKeyword(body:any,keywordOverride?:string){
...
function uiShadowConfig(body:any,keywordOverride?:string):SourcingConfig {
  ...
  return {
    ...
    enabled:false,
    mode:'shadow',
    keyword:normalizedKeyword(body,keywordOverride),
    maxCandidates,
    maxAiCalls:3,
    durationMs:40000,
...
function uiProductionCandidateConfig(body:any,keywordOverride?:string):SourcingConfig {
  ...
  return {
    ...
    durationMs:40000,
    retries:Math.min(policyConfig.retries,2),
    commercialProxiesAllowed:false,
    imageSaleUseAllowed:process.env.AUTOPILOT_CJ_IMAGE_SALE_USE_ALLOWED==='true',
    version:`${policyConfig.version}:production-revalidation-v1`,
  };
}
...
export function createSourcingRouter(
 ...
 router.get('/sourcing/status',requireControlPlaneAuth,async(_req,res)=>{
  try {const config=sourcingConfig(); const state=await storeFactory().command('status',{});
   ...
   return res.json({orchestrator:{enabled:config.enabled,shadowMode:config.mode==='shadow',lastRun:runs[0]||null,
    ...
    governance:{councilEnabled:true,autonomousPurchaseAllowed:false,autoPublicationEnabled:process.env.AUTOPILOT_V4_AUTO_PUBLISH_ENABLED==='true'},
 ...
 router.post('/sourcing/admin-shadow-run',requireSupabaseAdminAuth,async(req,res)=>{
  ...
  try {
   ...
   return res.json({
    ...result,manual:true,shadow:true,cronEnabled:false,policyVersion:UI_SHADOW_POLICY.policyVersion,
    transportPolicy:'mcp_first_rest_fallback',
    marketEvidence:{priority:['mercadolibre_uy','gemini_google_search','openrouter_web_search'],...marketProviderStatus()},
    searchQuery,
    safety:{checkout:false,purchases:false,autoPublish:false,maxCandidates:config.maxCandidates,maxAiCalls:config.maxAiCalls},inspection,
   });
  } catch(error:any) {
 ...
 // Manual release-candidate lane. It uses fresh CJ + Market Evidence in
 // production evidence mode, but deliberately stops after Council approval.
 ...
 router.post('/sourcing/admin-release-candidate',requireSupabaseAdminAuth,async(req,res)=>{
  ...
  try{
   const rawKeyword=typeof req.body?.keyword==='string'?req.body.keyword:'facial headband';
   const searchQuery=await optimizeCJSearchKeyword(rawKeyword);
   const config=uiProductionCandidateConfig(req.body,searchQuery.keyword);
   const result=await runner(storeFactory(),config,`admin-release-candidate:${randomUUID()}`);
   const inspection=await inspectShadowRun(result.run_id);
   return res.json({
   ...
   console.error('Admin release candidate run failed:',error?.message || error);
   return res.status(503).json({error:'ADMIN_RELEASE_CANDIDATE_RUN_INCOMPLETE'});
  }
 });
 ...
 router.get('/sourcing/admin-shadow-status',requireSupabaseAdminAuth,async(req,res)=>{
  try {
   ...
   return res.json({
    shadow:true,cronEnabled:sourcingConfig().enabled,transportPolicy:'mcp_first_rest_fallback',
    marketEvidence:{priority:['mercadolibre_uy','gemini_google_search','openrouter_web_search'],...marketProviderStatus()},
