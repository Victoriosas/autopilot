...
// independently retrieved variant, stock, freight and policy data may promote it.
...
export function buildCJLiveEvidence(input: {
  ...
  const candidate: OpportunityCandidate | undefined = variant && supplierCost !== null && shippingCost !== null && policyComplete
    ? {
      ...
      logisticsScore: freight ? logisticsScore(freight) : undefined,
      evidence: {
        title: 'observed',
        source: 'observed',
        demand: 'unknown',
        supplierReliability: 'unknown',
        logistics: freight ? 'observed' : 'unknown',
        competition: 'unknown',
      },
      metadata: {
        providerCurrency: config.providerCurrency,
